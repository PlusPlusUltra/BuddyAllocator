...
int isFree(BuddyAllocator* alloc, int idx)
{
	if(!getBit()) return 0;
	for (int i = 0; i < _; i++)
	{
		//controlla se padre è libero
		//se un padre è occupato return 0
		//int qualcosa = suo padre
	}
	for (int i = 0; i < _; i++)
	{
		//stessa cosa ma con i figli
	}
...
int minBuddy(BuddyAllocator* alloc, int level) //find the index of the smallest available buddy
{
	...
	{
		if (isFree(i) return i; //pseudo codice wise sistemata
	}
...
{
	setBit(alloc->tree,idx); //lol, sistemata
}
